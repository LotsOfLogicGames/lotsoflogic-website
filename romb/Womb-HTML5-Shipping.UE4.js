...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_Womb', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202602111722, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../Womb/Womb.uproject','-stdout',];
...
var canvasAspectRatioWidth = 960; // 1366;
var canvasAspectRatioHeight = 600; // 768;
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('Womb-HTML5-Shipping.wasm'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Womb-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Womb-HTML5-Shipping'+dataVariant+'.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Utility.js')).then(addScriptToDom);
		

		// --- CONFIGURATION: Define your original file and chunk names here ---
		var originalDataName = 'Womb-HTML5-Shipping' + dataVariant + '.data';
		var chunkList = [
			originalDataName + '.part1', // Make sure these match your actual chunk filenames
			originalDataName + '.part2',
			originalDataName + '.part3',
			originalDataName + '.part4'
		];
		// --------------------------------------------------------------------

		var dataDownload = Promise.all(chunkList.map(function(chunkName) {
			// We fetch every chunk in parallel
			return fetchOrDownloadAndStore(db, Module.locateFile(chunkName), 'arraybuffer');
		})).then(function(buffers) {
			// 1. Calculate the total size needed for the merged file
			var totalLength = 0;
			for (var i = 0; i < buffers.length; i++) {
				totalLength += buffers[i].byteLength;
			}

			// 2. Create a new array to hold the combined data
			var combinedArray = new Uint8Array(totalLength);
			var offset = 0;

			// 3. Copy each chunk into the combined array
			for (var i = 0; i < buffers.length; i++) {
				combinedArray.set(new Uint8Array(buffers[i]), offset);
				offset += buffers[i].byteLength;
			}

			// 4. Store it so UE4 can find it using the ORIGINAL filename
			Module['preloadedPackages'] = {};
			Module['preloadedPackages'][Module.locateFile(originalDataName)] = combinedArray.buffer;

			// 5. Continue the original loading chain
